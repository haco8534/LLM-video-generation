...
class ReadGenerator:
    ...
    _PROMPT =  '''
            以下の指示に厳密に従い、与えられた日本語のセリフ配列を同じ順序・要素数のひらがなだけの配列に変換してください。

            【指示】
            1. 出力は JSON 形式の文字列配列（[…]）のみとし、説明や余計な文章は含めない。
            2. 元のセリフ内の漢字はすべてひらがなに、カタカナや英語も音に合わせてひらがなに変換する。
            - 例：  
                - 「AI」→「えーあい」  
                - 「Tokyo」→「とうきょう」  
                - 「ニューヨーク」→「にゅーよーく」  
            3. 文末の句点「。」は入力の有無にかかわらず付けない。
            4. 変換後の文字数やトークン数は気にせず、正しい読み仮名を優先する。

            【入力】  
            ["こんにちは、今日のテーマはAIです。", "次に、最適化の話をしましょう。"]

            【出力】  
            ["こんにちは、きょうのてーまはえーあいです", "つぎに、さいてきかのはなしをしましょう"]
    '''
